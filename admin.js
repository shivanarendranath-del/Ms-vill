...
function openEditExpensesModal(){
  ...
  function rowHtml(r, i){
    return `
      <div class="member-row" style="align-items:flex-start; flex-wrap:wrap; gap:8px 10px;">
        ...
        <input type="text" inputmode="numeric" class="e-row-split" data-i="${i}" value="${r.split||0}" placeholder="Split Expenses" style="width:100px; margin-bottom:0;">
  ...
  function renderModal(){
    ...
    wrap.querySelector("#e-add-row").onclick = ()=>{
      draft.rows.push({ name:"", due:0, paid:0, split:0, status:"" });
      renderModal();
    };
    ...
    wrap.querySelector("#e-save").onclick = async ()=>{
      ...
      wrap.querySelectorAll(".e-row-split").forEach(el=> draft.rows[parseInt(el.getAttribute("data-i"),10)].split = Number(el.value)||0);
...
function openAddMemberModal(){
  ...
  wrap.querySelector("#save-member").onclick = async ()=>{
    ...
    const member = { username, name, password: DEFAULT_PASSWORD };
    state.members.push(member); // optimistic local echo
    const saved = await sappend("ms-villa:members", member, "username");
    if(saved !== null) state.members = saved; // authoritative list, including anyone else's concurrent additions
    wrap.remove();
    renderSettings();
  };
}
...
// Lets an admin edit which members are assigned to EVERY room — including
// both bedrooms — from one place in Settings, instead of having to open
// each room individually. Per-room editing (openAssignModal in app.js, from
// inside a room's own screen) still works too; this is the admin-only
// one-stop version.
function openManageRoomAssignmentsModal(){
  const draft = JSON.parse(JSON.stringify(state.rooms));

  function chipsFor(room, ri){
    return state.members.map(m=>{
      const on = room.assigned.includes(m.username);
      return `<div class="chip ${on?'on':''}" data-room-i="${ri}" data-chip="${m.username}">${m.name}</div>`;
    }).join("");
  }
  ...
  wrap.innerHTML = `
    <div class="modal">
      <h3>Manage Room Assignments</h3>
      <div class="foot-note" style="text-align:left; padding:0 0 12px; margin:0;">Tap names to add or remove them from each room, including both bedrooms, then save.</div>
      ${draft.map((room,ri)=>`
        <div class="section-title" style="padding:0; margin:${ri===0?'0':'18px'} 0 6px;">${room.name}</div>
        <div class="chip-select" data-room-chips="${ri}">${chipsFor(room, ri)}</div>
      `).join("")}
      <div class="error" id="mra-err" style="display:none;"></div>
      <button class="btn-primary" id="mra-save" style="margin-top:10px;">Save All Rooms</button>
      <button class="btn-ghost" id="mra-cancel">Cancel</button>
    </div>
  `;
  document.body.appendChild(wrap);
  ...
  wrap.querySelectorAll("[data-chip]").forEach(chip=>{
    chip.onclick = ()=>{
      const ri = parseInt(chip.getAttribute("data-room-i"),10);
      const u = chip.getAttribute("data-chip");
      const room = draft[ri];
      if(room.assigned.includes(u)){ room.assigned = room.assigned.filter(x=>x!==u); chip.classList.remove("on"); }
      else { room.assigned.push(u); chip.classList.add("on"); }
    };
  });
  wrap.querySelector("#mra-cancel").onclick = ()=> wrap.remove();
  wrap.querySelector("#mra-save").onclick = async ()=>{
    state.rooms = draft;
    const ok = await sset("ms-villa:rooms", state.rooms);
    if(!ok){
      const err = wrap.querySelector("#mra-err");
      err.style.display = "block"; err.textContent = "Couldn't save — check your connection and try again.";
      return;
    }
    wrap.remove();
    renderSettings();
    notifyMembers("Room assignments updated", "An admin just changed who's assigned to one or more rooms.");
  };
}

// Promotes (or demotes) a member's admin status. Always leaves at least one
// admin in the house so nobody can accidentally lock everyone out of the
// admin-only screens.
async function toggleMemberAdmin(username){
  const member = state.members.find(m=>m.username===username);
  if(!member) return;
  const admins = state.members.filter(m=>m.admin);
  if(member.admin && admins.length<=1){
    alert("Ms Villa needs at least one admin — make someone else admin first.");
    return;
  }
  const makingAdmin = !member.admin;
  if(!confirm(makingAdmin ? `Make ${member.name} an admin?` : `Remove admin access from ${member.name}?`)) return;
  member.admin = makingAdmin;
  const ok = await sset("ms-villa:members", state.members);
  if(!ok){ alert("Couldn't save — check your connection and try again."); return; }
  renderSettings();
  if(makingAdmin) notifyMembers("New admin added", `${member.name} is now an admin of Ms Villa.`);
}
